...
impl BandwidthAllocator for MLTCPTopo {
    ...
    fn allocate(&self, topo: &impl Topology, active_desc: &IndexMap<FlowId, FlowDesc>, active_state: &IndexMap<FlowId, FlowState>) -> Vec<f64> {
        ...
        // --- Compute per-job current-iteration progress ratios and print sorted order ---
        ...
        // Build ratios with deterministic per-call tie-break keys and per-job completed-flow counts
        let mut job_ratios: Vec<(usize, f64, u64, u64)> = Vec::new();
        ...
        for (jid, per_iter_total_job) in per_iter_total_by_job.iter() {
            ...
            let ratio = if *per_iter_total_job == 0 { 0.0 } else { (prog as f64) / (*per_iter_total_job as f64) };
            let tie_key = self.tie_key(round, *jid * 5 + 10);
            let completed = ctx.flow_completions_per_job.borrow().get(jid).copied().unwrap_or(0);
            job_ratios.push((*jid, ratio, completed, tie_key));
        }

        job_ratios.sort_by(|(_id_a, _r_a, c_a, k_a), (_id_b, _r_b, c_b, k_b)| {
            match c_a.partial_cmp(c_b).unwrap_or(Ordering::Equal) {
