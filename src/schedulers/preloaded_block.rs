...
/// A scheduler that combines fixed placements for preloaded jobs with
/// block-based scheduling for dynamically arriving jobs.
///
/// Preloaded jobs (e.g., from an initial cluster state snapshot) have their
/// host assignments set before the simulation starts. New jobs arriving from
/// a trace are scheduled by the inner `BlockScheduler`.
#[derive(Debug)]
pub struct PreloadedBlockScheduler {
    job_queue: VecDeque<JobId>,
    preloaded_placements: HashMap<JobId, Vec<usize>>,
    inner: BlockScheduler,
}

impl PreloadedBlockScheduler {
    ...
    /// Register a fixed placement for a preloaded job.
    /// `hosts` must be sorted and have length == num_workers.
    pub fn set_preloaded_placement(&mut self, job_id: JobId, hosts: Vec<usize>) {
        self.preloaded_placements.insert(job_id, hosts);
    }
}

impl JobScheduler for PreloadedBlockScheduler {
